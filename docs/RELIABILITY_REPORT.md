...
**Осталось (страховки, требуют UX-решений/устройства — делать с Артемием):**
- P1-4 детект пропуска + карточка «не прозвенел» (среднее);
- P1-3 самотест «Проверить будильник» (среднее, UI);
- P1-1 вендор-инструкции по производителю (среднее, контент + интенты, проверять на Xiaomi);
- P0-3 **слой B** (direct-boot-aware путь сигнала) — крупное, ночной-ребут проверка;
- P2-1 — низкий приоритет (дрейф лечится reschedule-на-старте).

Пункты ✅ (P0-2, P0-4, P1-2, P1-5, P2-2, P2-3) — доработок не требуют.

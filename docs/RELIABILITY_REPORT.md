...
|---|---|---|---|---|
| P0-1 | Право на точные будильники | ⚠️ | `AndroidManifest.xml:6-7`, `AlarmPermissions.kt:21-24,57-61`, `AlarmReadiness.kt:39-42` | Манифест и готовность — полностью ✅ (USE_EXACT_ALARM + SCHEDULE_EXACT_ALARM maxSdk32, критичный баннер, интент в настройки). **Пробел:** в `AlarmScheduler.scheduleAt` (`:141-161`) нет guard `canScheduleExactAlarms()` и нет try-catch вокруг `setAlarmClock`. На API 31-32 без разрешения `setAlarmClock` бросит `SecurityException`; в пакетном пути ловит `AlarmRescheduleBatch`, но в одиночном `reschedule` (из редактора) исключение уйдёт наверх. **Делать:** обернуть `setAlarmClock` в try-catch `SecurityException` (+ событие `ERROR`/`SIGNAL_DEGRADED` в журнал) или guard на 31-32. |
...
| **P0-3** | **Direct Boot (ночной ребут залоченным)** | **❌** | `AndroidManifest.xml:80-90`, `SystemRescheduleActions.kt:10` | **Главный пробел.** У `BootReceiver` нет `directBootAware="true"`, не слушается `ACTION_LOCKED_BOOT_COMPLETED`, нет DE-кэша (`createDeviceProtectedStorageContext()` не встречается). После ночного OTA-обновления/ребута до ввода пин-кода Room недоступна — будильники не перевыставятся, человек проспит. Отложено осознанно (`SHIFTALARM_PROJECT_REVIEW…`). **Делать:** directBootAware-ресивер на `LOCKED_BOOT_COMPLETED`, кэш «ближайшие звонки 7 дней» в DE-хранилище (DataStore/prefs), слепое перевыставление из кэша, полный пересчёт по `USER_UNLOCKED`/первому старту, юнит-тест сериализации кэша. |
...
| P0-5 | Foreground-сервис на 34+ | ⚠️ | `AlarmService.kt:115-123,247-256`, `AlarmReceiver.kt:41,61,97`, `AndroidManifest.xml:14,77` | `startForegroundService` + `startForeground(type=MEDIA_PLAYBACK)` ✅; передача ресивер→сервис под `goAsync()` ✅. **Пробел:** `PARTIAL_WAKE_LOCK` не удерживается ни при передаче, ни во время проигрывания (grep `WakeLock`=0), хотя `WAKE_LOCK` в манифесте объявлен — на части устройств звук может оборваться при погашенном экране. **Доводка targetSdk 36:** тип `mediaPlayback` рабочий (сервис реально играет), но для будильника с `USE_EXACT_ALARM` более профильный тип — `systemExempted`; зафиксировать выбор. **Делать:** держать `PARTIAL_WAKE_LOCK` в сервисе, пока играет; рассмотреть `systemExempted`. |

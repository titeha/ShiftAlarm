...
## Планы развития (после запуска 1.0)

| Документ | О чём |
|----------|-------|
| [DAY_GREETINGS.md](DAY_GREETINGS.md) | ТЗ v1.1 «Настроение дня»: праздник дня + фраза дня карточкой после «Стоп» (+ опц. уведомление на отдельном канале). Оффлайн из assets, чистый `DayGreetingResolver` + валидатор. Надёжность звонка не трогает. Контент собирает владелец. |
| [MONETIZATION.md](MONETIZATION.md) | Стратегия монетизации: надёжность бесплатна навсегда; ядро бесплатно; платит бригадир за живой общий график («Бригада», подписка L2); лестница шаринга L0→L1→L2; донаты; реклама только после 10–20к DAU; рельсы по сторам через flavors; что заложить в код уже сейчас (конверт с `v`, `uuid`/`updatedAt`/`origin`). Требует сильного анализа — отложено. |
